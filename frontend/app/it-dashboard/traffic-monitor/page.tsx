...
export default function ITTrafficMonitorPage() {
  ...
  const hasAccess = useMemo(() => {
    const user = getITUser();
    if (!user) return false;
    return IT_USER_TYPES.has(normalizeUserType(user.user_type));
  }, []);
  ...
  useEffect(() => {
    ...
    if (!hasAccess) {
      setError("IT Support access required.");
      setLoading(false);
      return;
    }
    ...
    const fetchData = async () => {
      try {
        ...
        if (err.response?.status === 401) {
          ...
          clearITAuth();
          setTimeout(() => router.push("/it-login"), 1200);
          return;
        }
